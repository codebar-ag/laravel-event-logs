...
- `config/laravel-event-logs-exclude-routes-defaults.php` for the default Nova/Livewire `exclude_routes` list; published alongside `laravel-event-logs.php` under tag `laravel-event-logs-config`.
...
- Republish config to receive `laravel-event-logs-exclude-routes-defaults.php` if you rely on published files.
